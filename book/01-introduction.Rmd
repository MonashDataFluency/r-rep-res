...
This is a stand-alone language that is used in a variety of places, its main advantage is that it can be easily ready by humans as well easily parsed by computers. A lot of the time YAML can be used to add a configuration file. This is an example of how it is used with Rmarkdown. We will talk about [YAML in more depth in a different section](#yaml-header-introduction). In brief, we will use YAML to set a document's appearance and link additional files with the documents, such as bibliographies.
